...
!.....
      ...
      CALL GELI(REC,GAP2)
      CALL VPDASH
      IF (DABS(DASH1).GT.1D-12.AND.DABS(GAP1).GT.1D-12.AND.DABS(DASH2).GT.1D-12 &
      .AND.DABS(GAP2).GT.1D-12) THEN
        LINDASH=.TRUE.
      ELSE
        LINDASH=.FALSE.
      END IF
...
!.....
      ...
      CALL GELI(REC,L)
      CALL GELI(REC,R)
      IF (DABS(L-R).LT.1D-12) THEN
       L=L*0.9D0
       R=R*1.1D0
       IF (DABS(L).LT.1D-12) THEN
        L=-1.0D0
        R=1.0D0
       END IF
      END IF
      CALL GELI(REC,U)
      CALL GELI(REC,O)
      IF (DABS(U-O).LT.1D-12) THEN
       U=U*0.9D0
       O=O*1.1D0
       IF (DABS(U).LT.1D-12) THEN
        U=-1.0D0
        O=1.0D0
       END IF
      END IF
...
!.....
      ...
      END IF
!.....
      IF (IDENT.EQ.'ARC') THEN
      CALL ARC1
      GOTO 10
      END IF
!.....
      IF (IDENT.EQ.'BEZ3') THEN
      CALL BEZ3
      GOTO 10
      END IF
!.....
...
!----- end of common variables
      ...
      INTEGER(4) I,II,I1,I2,I3,LAE,C1,C2,J1,J2,C4
!==== (C1 is only used in YAXIS to define distance of label to numbers)
      C4=C1
      THETA=0.0D0
      ZERO=0.0D0
      I=I3
...
!====
      ...
      IF (DABS(GRID-1.0D0).LT.1D-12.OR.DABS(GRID-10.0D0).LT.1D-12) THEN
      II1=2
      II2=2
      END IF
      IF (DABS(GRID-2.0D0).LT.1D-12) THEN
      II1=2
      II2=4
      END IF
      IF (DABS(GRID-2.5D0).LT.1D-12.OR.DABS(GRID-5.0D0).LT.1D-12) THEN
      II1=5
      II2=5
      END IF
...
!====
      ...
      CALL GELI(REC,GRS)
      CALL KOOLI(X,Y,Z)
   10 IF (DABS(X-999.0D0).LT.1D-12.AND.DABS(Y-999.0D0).LT.1D-12) GOTO 999
      CALL XXYY(X,Y,Z,XX,YY)
...
!====
      ...
      I1=1
      I2=0
      IF (DABS(F1-1.0D0).LT.1D-12) I1=0
      IF (F1.GT.0) I2=1
      CALL SVGPATH(I1,I2)
   ...
   10 IF (DABS(X-999.0D0).LT.1D-12.AND.DABS(Y-999.0D0).LT.1D-12) GOTO 999
      ...
      GOTO 10
  999 CONTINUE
      IF (DABS(F3).GT.1D-12) CALL VPDRAW(XX,ZERO)
      IF (DABS(F2).GT.1D-12) CALL VPCLOSE
      IF (DABS(F1).LT.1D-12) CALL VPSTROKE
      IF (DABS(F1-1.0D0).LT.1D-12) CALL VPFILL
      IF (DABS(F1-2.0D0).LT.1D-12) CALL VPFLLSTR
      CALL SVGPATHEND
      RETURN
      END
!
!***************
      ...
      INCLUDE 'expl.cmn'
!----- end of common variables
      REAL(8) RX,RY,ROT,F1,F2,X1,Y1,X2,Y2,X,Y,Z
      INTEGER(4) I1,I2
!====
      CALL GELI(REC,RX)
      CALL GELI(REC,RY)
      ...
      CALL VPMOVE(X1,Y1)
      CALL VPARC1(RX,RY,ROT,F1,F2,X2,Y2)
!
      CALL SVGPATHEND
      RETURN
      END
...
!***************
      ...
      INCLUDE 'expl.cmn'
!----- end of common variables
      REAL(8) X0X,X0Y,X1X,X1Y,X2X,X2Y,XX,YY,X,Y,Z
      INTEGER(4) I1,I2
!====
      CALL KOOLI(X,Y,Z)
      CALL XXYY(X,Y,Z,X0X,X0Y)
      CALL GELI(REC,X1X)
      CALL GELI(REC,X1Y)
      CALL GELI(REC,X2X)
      CALL GELI(REC,X2Y)
      ...
      CALL VPMOVE(X0X,X0Y)
      CALL VPBEZ3(X1X,X1Y,X2X,X2Y,XX,YY)
!
      CALL SVGPATHEND
      RETURN
      END
...
!====
   ...
   10 CALL GELI(REC,X)
      CALL GELI(REC,Y)
      IF (DABS(X-999.0D0).LT.1D-12.AND.DABS(Y-999.0D0).LT.1D-12) GOTO 999
      CALL GELI(REC,F1)
      CALL GELI(REC,F2)
      CALL XXYY(X,Y,Z,XX,YY)
      CALL SYMBOL(XX,YY,SYM,GRS)
...
!====
    ...
    1 CALL TAXI(REC,TEXT)
      ...
      SSYM=SYM
      IF (DABS(FILL-1.0D0).LT.1D-12) SSYM=-SYM
      GRZ=GRS/5.0D0
...
!====
      ...
      CALL KOOLI(X,Y,Z)
      DO 500,I=1,500
      IF (DABS(X-999.0D0).LT.1D-12.AND.DABS(Y-999.0D0).LT.1D-12) GOTO 10
      CALL XXYY(X,Y,Z,XX,YY)
      XZUG(I)=XX
      YZUG(I)=YY
      CALL KOOLI(X,Y,Z)
...
!====
      IF (SYSTEM.EQ.3) THEN
      SUMME=X+Y+Z
      IF (DABS(SUMME).LT.1D-12) SUMME=1.0D0
!      IF (ABC.EQ.'ABC') THEN
      CA=X/SUMME
      CB=Y/SUMME
      CC=Z/SUMME
...
!-----
      ...
      IF (B.LE.0.0D0) B=15.0D0
      IF (H.LE.0.0D0) H=15.0D0
      IF (DABS(XMIN-XMAX).LT.1D-12) THEN
       XMIN=XMIN*0.9D0
       XMAX=XMAX*1.1D0
       IF (DABS(XMIN).LT.1D-12) THEN
        XMIN=-1.0D0
        XMAX=1.0D0
       END IF
      END IF
      IF (DABS(YMIN-YMAX).LT.1D-12) THEN
       YMIN=YMIN*0.9D0
       YMAX=YMAX*1.1D0
       IF (DABS(YMIN).LT.1D-12) THEN
        YMIN=-1.0D0
        YMAX=1.0D0
       END IF
      END IF
...
!     WRITE (6,1011) XPOS,YPOS,SIZLAB,ANGE,NCHAR,LABEL
 1011 FORMAT (2(0PE20.12),F10.7,F10.4,I5,A500)
      END IF
!-----
      IF (DABS(XPOS).LT.1D-12.AND.DABS(YPOS).LT.1D-12.AND.LABEL.EQ.'97') THEN
      X98=B+1.0D0
      Y98=0.25D0
      ...
      END IF
!-----
      IF (DABS(XPOS).LT.1D-12.AND.DABS(YPOS).LT.1D-12.AND.LABEL.EQ.'98') THEN
      X98=B+1.0D0
      Y98=0.25D0
...
!<image x="141.7" y="297.36" width="283.46" height="212.6" xlink:href="Base(5)_export.bmp">

      IF (IMAN.GT.0) THEN
      B1=IMAB*CMPX
      H1=IMAH*CMPX
      X1=(IMAX+X0)*CMPX
      Y1=595.0D0-(IMAY+Y0+IMAH)*CMPX
      CALL LABLA(IMACH,I1)
      WRITE (svg,1007) X1,Y1,B1,H1,IMACH(1:I1)
 1007 FORMAT (/'<image x="',F7.2,'" y="',F7.2,'" width="',F7.2, &
      '" height="',F7.2,'" xlink:href="',A,'"')
      END IF
...
!====
      IF (MAKEPS) THEN
      IF (DABS(DASH1).LT.1D-12.AND.DABS(GAP1).LT.1D-12.AND.DABS(DASH2).LT.1D-12 &
      .AND.DABS(GAP2).LT.1D-12) THEN
      WRITE (pst,1000)
 1000 FORMAT ('[] 0 dash')
      ELSE
...
!
!***************
      SUBROUTINE VPARC1(RX,RY,ROT,F1,F2,X2,Y2)
      IMPLICIT NONE
      INCLUDE 'expl.cmn'
!----- end of common variables
      REAL(8) RX,RY,ROT,F1,F2,X2,Y2
      INTEGER(4) I1,I2
!====
      IF (MAKESVG) THEN
      I1=0
      I2=0
      IF (DABS(F1).GT.1D-12) I1=1
      IF (DABS(F2).GT.1D-12) I2=1
      WRITE(svg,1005) RX*CMPX,RY*CMPX,ROT,I1,I2, &
      (X2+X0)*CMPX,595.0D0-(Y2+Y0)*CMPX
 1005 FORMAT ('A ',F10.4,1X,F10.4,1X,F10.4,I4,I4,1X,F10.4,1X,F10.4)
      END IF
!
      RETURN
      END
!
!***************
      SUBROUTINE VPBEZ3(X1X,X1Y,X2X,X2Y,XX,YY)
      IMPLICIT NONE
      INCLUDE 'expl.cmn'
!----- end of common variables
      REAL(8) X1X,X1Y,X2X,X2Y,XX,YY,Q1,Q2,Q3,Q4,Q5,Q6
!====
      IF (MAKESVG) THEN

      Q1=(X1X-X0)*CMPX
      Q2=595.0D0-(X1Y+Y0)*CMPX
      Q3=(X2X-X0)*CMPX
      Q4=595.0D0-(X2Y+Y0)*CMPX
      Q5=(XX-X0)*CMPX
      Q6=595.0D0-(YY+Y0)*CMPX

      WRITE(svg,1005) (X1X+X0)*CMPX,595.0D0-(X1Y+Y0)*CMPX, &
      (X2X+X0)*CMPX,595.0D0-(X2Y+Y0)*CMPX, &
      (XX+X0)*CMPX,595.0D0-(YY+Y0)*CMPX
 1005 FORMAT ('C ',F10.4,1X,F10.4,1X,F10.4,1X,F10.4,1X,F10.4,1X,F10.4)

      WRITE (6,2000) X1X,X1Y,X2X,X2Y,XX,YY
 2000 FORMAT ('C ',F10.4,1X,F10.4,1X,F10.4,1X,F10.4,1X,F10.4,1X,F10.4)
      WRITE (6,2005) X0,Y0,CMPX
 2005 FORMAT ('X0,Y0,CMPX ',F10.4,1X,F10.4,1X,F10.4,1X,F10.4,1X,F10.4,1X,F10.4)
...
!
      ...
      ELSE
       ...
       IDINT(FCOLG*255.0D0),IDINT(FCOLB*255.0D0)
      END IF
