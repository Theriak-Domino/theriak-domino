!therter.f90  Version: 2023.06.06
! -----------------------------------------------------------------------
...
!.....
      ...
      DO 500,N=1,NTIE
      KK=0
      DO K=FIRSTP(N),LASTP(N)
        KK=KK+1
        DO I=1,3
          ZUG(KK,I)=TIEX(K,1,I)
        END DO
      END DO
      DO K=LASTP(N),FIRSTP(N),-1
        KK=KK+1
        DO I=1,3
          ZUG(KK,I)=TIEX(K,2,I)
        END DO
...
!-----
      IMIN=1
      DO II=1,3
        XXT(II)=TIEX(NRL,NPT,II)
      END DO
      DO 500,I=1,NZ
      SOV(I)=0.0D0
      DO II=1,3
        SOV(I)=SOV(I)+(XXT(II)-ZUG(I,II))**2
      END DO
      IF (I.EQ.1) THEN
      SOVMIN=SOV(1)
      IMIN=1
...
!******************************
      ...
      A3,B3,C3,DD,XM,YM,LL,AA1,BB1,CC1,AA2,BB2,CC2,C4
      C4=A1
      C4=A2
      S=2.0D0/DSQRT(3.0D0)
...
!-----
 ...
 3000 FORMAT (/' corners not linearly independent')
      ...
      END DO
      DO I=1,I001
        XCORN(I,K)=XCORN(I,IIX)
      END DO
      DO I=1,I001
        XCORN(I,IIX)=XCORN(I,0)
      END DO
  100 CONTINUE
      F=XCORN(K,K)
