...
!      IF (XMIN.EQ.XMAX.AND.XVAR.NE.'TER') THEN
      IF (DABS(XMIN-XMAX).LT.1D-12.AND.XVAR.NE.'TER') THEN
      IF (MULTI) STOP
      CALL SHOUTI
      WRITE (6,1032)
 1032 FORMAT (//' X-min is equal to X-max')
      ...
      END IF
!---
      IF (XVAR.EQ.'P'.AND.DABS(XMIN).LT.1D-12) XMIN=0.0001
      IF (XVAR.EQ.'P'.AND.DABS(XMAX).LT.1D-12) XMAX=0.0001
!---
...
!>
 ...
 1042 FORMAT (//' Y-min is equal to Y-max')
      ...
      END IF
!---
      IF (YVAR.EQ.'P'.AND.DABS(YMIN).LT.1D-12) YMIN=0.0001
      IF (YVAR.EQ.'P'.AND.DABS(YMAX).LT.1D-12) YMAX=0.0001
!-----
...
!-----
  250 CONTINUE
!----- here FUNTAXI
      IF (DABS(ISODIV-1.0D0).GT.1D-12) THEN
...
!----- CHECK FOR BUMPS
      ...
      IF (DABS(F1).GT.1D-12.AND.DABS(F2).GT.1D-12) THEN
...
!*****
      ...
      F001=VOLSOL
      IF (DABS(VOLSOL).LT.1D-12) F001=1.0D0
      IF (MAPEM.EQ.-3) XMAP(MAPANZ)=WTSOL/F001
      IF (MAPEM.EQ.-4) XMAP(MAPANZ)=GGTOT
      F001=WTSOL
      IF (DABS(WTSOL).LT.1D-12) F001=1.0D0
...
!-----
      ...
      F001=VOLPH(IP)
      IF (DABS(F001).LT.1D-12) F001=1.0D0
...
!===  das Folgende ist ein wenig unsauber
  150 CONTINUE
!----- hier ISODIV
      IF (DABS(ISODIV-1.0D0).GT.1D-12.AND.DABS(ISODIV).GT.1D-12) &
...
!-----
      ...
      EMCODE(NUN2)=0
!cdcmar08
      STPHNAM(NUN2)=MAPTEX(NPRO)(1:LEN(STPHNAM(NUN2)))
      SHORTNAM(NUN2)=MAPTEX(NPRO)(1:LEN(SHORTNAM(NUN2)))
      END IF
!*****
      MORE=.TRUE.
      DO 500,IA=1,NASS
