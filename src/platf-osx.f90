!platf-osx.f90  Version: 29.11.2022-dev
! -----------------------------------------------------------------------
...
!      ETOTAL=EARRAY(2)
!      ETOTAL=EVALUE
      CALL CPU_TIME(ETOTAL)
      EHRS=INT(ETOTAL/3600.0)
      ETOTAL=ETOTAL-3600.0*EHRS
      EMIN=INT(ETOTAL/60.0D0)
      ETOTAL=ETOTAL-60.0*EMIN
      ESEC=INT(ETOTAL)
!-----
...
!*********************************************************************
!
      SUBROUTINE oldGetLineArgs (i,largum,ierr)
...
!---- (arg 0 = program name (includes path if program invoked by double clicking))
      ...
      CHARACTER*(*) largum
      ierr=0
      CALL get_command_argument(i, largum)
      la=LEN_TRIM(largum)
      IF (la == 0) THEN
        ierr=1
      ELSE
        IF (ICHAR(largum(la:la)).EQ.13) largum(la:la)=' '
      END IF
!-----
      RETURN
      END
