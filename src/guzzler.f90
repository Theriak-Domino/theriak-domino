!guzzler.f90  Version: 2023.03.06
! -----------------------------------------------------------------------
...
!      DO I=1,larg
!        WRITE (UNIT=6,FMT='(''arg: '',i2,1x,a)') I,largum(I)
!      END DO
!-----
...
!-----
      ...
      NCHZU(I),LABZU(I)
 1002 FORMAT (2(0PE20.12),0PE10.3,0PE10.3,I5,A150)
      ELSE
 ...
 1001 FORMAT (2(0PE10.3),0PE10.3,0PE10.3,I5,A150)
      END IF
!-----
!!eq      IF (XZU(I).EQ.0.0D0.AND.YZU(I).EQ.0.0D0.AND.LABZU(I).EQ.'97') &
      ...
      END IF
!-----
!!eq      IF (XZU(I).EQ.0.0D0.AND.YZU(I).EQ.0.0D0.AND.LABZU(I).EQ.'98') &
...
!----- UPDATE DANGER
      ...
      DO I1=1,LASTRE
        DANGER(I1)=MAX0(DANGER(I1),GITTER(1,IXX(I1),IYY(I1)))
      END DO
...
!*****
      ...
      I2=0
   50 DO I1=I2+1,100
        IF (ARCH(I1:I1).NE.' '.AND.ARCH(I1:I1).NE.',') GOTO 51
      END DO
   51 IF (I1.GT.100) GOTO 53
      DO I2=I1+1,100
        IF (ARCH(I2:I2).EQ.' '.OR.ARCH(I2:I2).EQ.',') GOTO 52
      END DO
   52 NFORCE=NFORCE+1
      HILCH=' '
...
!-----
      ...
      DO NAIM=NAIMNR,NAIMAX
        CALL SEARCH(LL,NAIM,I1,I2,XL)
        IF (XL.GE.F1) GOTO 106
      END DO
      IF (NFCODE.EQ.2) THEN
      I1=FIRST(LL)
      I2=LAST(LL)
...
!
  801 IF (CODE(LL).EQ.0) THEN
      ...
      DO NAIM=NAIMAX,MATE+1
        CALL SEARCH(LL,NAIM,I1,I2,XL)
        IF (XL.GE.DMIN) GOTO 112
      END DO
...
!-----
 ...
 1996 FORMAT ( &
      ...
      IF (LONGCOD) THEN
      WRITE (cln,1997)
 1997 FORMAT ('FAT   0.015'/'NPLOG2  ')  !dec line thickness from 0.03 to 0.015
      ELSE
      WRITE (cln,1998)
 1998 FORMAT ('FAT   0.015'/'NPLOIG  ')  !dec line thickness from 0.03 to 0.015
      END IF
...
!-----
      IF (LONGCOD) THEN
      FORM='(2(1PE20.12),0PF10.7,F10.4,I5,A150)'  !inc 100->150 len of bulk comp line
      ELSE
      FORM='('//FORXY//',0PF10.7,F10.4,I5,A150)'  !inc 100->150 len of bulk comp line
      END IF
      CALL LABLA(FORM,ILF)
!-----
      DO 590,I=1,NZU
...
!***********************************************************
      ...
      IMPLICIT NONE
      INTEGER(4) MAXGRD,MATE
      PARAMETER (MAXGRD=100,MATE=2)
!-----
      INTEGER(4) GITTER(0:1,-1:MAXGRD,-1:MAXGRD), &
      TEMPLE(-MATE:MATE,-MATE:MATE),NXGRID,NYGRID
      COMMON /GITT/ GITTER,TEMPLE,NXGRID,NYGRID
...
!-----
      ...
      DO II=I001,I002
        ...
        GITTER(LOC,IX,II)=MAX0(GITTER(LOC,IX,II),TEMPLE(0,II-IY))
      END DO
      DO II=I003,I004
        IF (LOC.EQ.0) GITTER(1,II,IY)=0
        GITTER(LOC,II,IY)=MAX0(GITTER(LOC,II,IY),TEMPLE(II-IX,0))
      END DO
      RETURN
      END
!
...
!-----
  ...
  222 IF (I1.GT.LAST(L)) GOTO 999
      DO IN1=I1,LAST(L)
        IF (DANGER(IN1).LT.N) GOTO 333
      END DO
  333 IF (IN1.GT.LAST(L)) GOTO 999
      DO IN2=IN1+1,LAST(L)
        IF (DANGER(IN2).GE.N) GOTO 444
      END DO
  444 IN2=IN2-1
...
!-----
      ...
      END DO
!!eq  222 IF (LAE(I)-LAE(I-1).EQ.0.0D0) THEN
