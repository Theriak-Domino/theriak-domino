!theriaq.f90  Version: 29.11.2022-dev
! -----------------------------------------------------------------------
...
!----- CC0(ncol+1)=ZSUM-ZWERT
      ...
      DO IR=1,NROW
        QSUM(IR)=0.0D0
        DO IC=1,NCOL
          QSUM(IR)=QSUM(IR)+AA(IR,IC)*MOLW(IC)
        END DO
      END DO
...
!----- d(XRES)/d(Ln(m)
      ...
      DO IR=1,NCOL
        JACOB(IR,IC)=(XRES(IR)-CC0(IR))/(FINDIFF)
      END DO
...
!-----
      DO I=1,NMAX
        GCORR(I)=G(I)
        DO II=1,NUN
          GCORR(I)=GCORR(I)+X(I,II)*MUEG(II)
        END DO
      END DO
...
!----
      ...
      DO I=1,100
        NBASIS=NBASIS+1
        READ (UNIT=15,FMT='(A16)',END=444) BASIS(NBASIS)
      END DO
  444 NBASIS=NBASIS-1
!----
      NROW=NUN
